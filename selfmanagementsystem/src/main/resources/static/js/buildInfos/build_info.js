setTimeout(function() {
var str = $("#BuildInfo [name='buildAddress']").text();
var str1 = str.replace($("#BuildInfo [name='address']").val(), '');
$("#BuildInfo [name='buildAddress']").text(str1)
}, 200);
...
$(function() { // 文档加载，执行一个函数	
			...
			function(e) {// 点击提交之后
				...
				if (bv.isValid()) {
					 var data = {};
		                ...
		                $.each(t, function() {                               
		                		if(this.name=="buildUse"){
		                    		...
		                    		data['buildImage']=$('#pic').attr('src');
		                    		
		                    	}else if(this.name=="buildAddress"){                   		
		                    		data['buildAddress']=$("#BuildInfo [name='address']").val()+this.value;
					...
					// 按钮
						...
						// 使用Ajax提交form表单数据
						$.ajax({
							url : "/build/edit?buildCode=" + getUrlParam("buildCode"),
							type : "put",
	...
	var str=$("input[name='refugeFloorLocation']").val();
	var arr = new Array();
	arr = str.split(',');
    var text1 = $.map(arr,function(value) {
        return value+"层";
    }).join(',');
	$("#BuildInfo :input[name='refugeFloorShow']").val(text1);
	...
	//input显示避难层数
	$("#BuildInfo :input[name='refugeFloorShow']").focus(function(){
		  
		$(this).attr("disabled","disabled"); 
		var a= $("#BuildInfo :input[name='groundLayerNumber']").val();
		...
		for(var i=1;i<=a;i++){
			var txt='<div class="checkbox checkbox-inline"><input type="checkbox" name="refugeFloor" id="refugeFloorLocation'+i+'" value="'+i+'"><label for="refugeFloorLocation'+i+'">'+i
			   +'层</label></div>'; 
			 $div+=txt;	
			}	
		$div+='</div></div></div>'
			...
			layer.open({
				  ...
				  success: function(layero, index){
					  var str = $(
						...
						arr = str.split(',');
					  for (key in arr) {
						  $("input[name='refugeFloor']").eq(arr[key]-1).attr("checked","checked");
				        }
					  $('[name="refugeFloor"]').click(function() {
					        text = $('[name="refugeFloor"]:checkbox:checked').map(function(index,elem) {
					            return $(elem).val();
					        }).get().join(',');
					        text1 = $('[name="refugeFloor"]:checkbox:checked').map(function(index,elem) {
					            return $(elem).val()+"层";
					        }).get().join('、');
					       //alert("选中的checkbox的值为："+text1);
					        var arr = new Array();
					    	arr = text.split(',');	
					        parent.$("input[name='refugeFloorShow']").val(text1);
					        parent.$("input[name='refugeFloorLocation']").val(text);	
					        parent.$("input[name='refugeFloorNumberShow']").val(arr.length);
					        parent.$("input[name='refugeFloorNumber']").val(arr.length);
					    });		 	
					  }
				});
