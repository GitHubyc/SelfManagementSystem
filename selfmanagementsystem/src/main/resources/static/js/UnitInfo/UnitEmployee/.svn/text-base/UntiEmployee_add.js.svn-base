    ...
    .bootstrapValidator({
        ...
        if( bv.isValid()){
            var data = {};
            var t = $('#UnitEmployee_add').serializeArray();
            $.each(t, function() {
             data[this.name] = this.value;
          });
    		$.ajax({
     		    url: '/unitEmployee/add',
     		    type: 'post',	
    ...
    $('#UnitEmployee_add').bootstrapValidator('addField', "documentNo1");
    $('#UnitEmployee_add').bootstrapValidator('addField', "documentNo2");
    $('#UnitEmployee_add').bootstrapValidator('addField', "documentNo3");

	$("#UnitEmployee_add select[name='documentType']").change(function(){	
		var num=$(this).val();
		var $templateEle = $("#UnitEmployee_add .form_content"),
