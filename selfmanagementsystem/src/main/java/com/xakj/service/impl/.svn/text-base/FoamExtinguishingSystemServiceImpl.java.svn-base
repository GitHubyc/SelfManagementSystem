package com.xakj.service.impl;

import java.util.HashMap;

import org.springframework.stereotype.Component;

import net.sf.json.JSONObject;

import com.xakj.service.FoamExtinguishingSystemService;

/**
 *描述:泡沫灭火系统
 *作者:袁文彪
 *时间:2017-08-08 14:04:09
 */
@Component
public class FoamExtinguishingSystemServiceImpl implements FoamExtinguishingSystemService{

	@Override
	public JSONObject list(HashMap<String, Object> paramMap) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public long add(HashMap<String, Object> paramMap) {
		// TODO Auto-generated method stub
		return 0;
	}

	@Override
	public long del(HashMap<String, Object> paramMap) {
		// TODO Auto-generated method stub
		return 0;
	}

	@Override
	public long edit(HashMap<String, Object> paramMap) {
		// TODO Auto-generated method stub
		return 0;
	}

	@Override
	public long details(HashMap<String, Object> paramMap) {
		// TODO Auto-generated method stub
		return 0;
	}

}
