...
function normalizeTimeTickInterval(tickInterval, unitsOption) {
	...
	if (interval === timenuits[YEAR] && tickInterval < 5 * interval) {
		multiples = [1, 2, 5];
	}
...
function getTimeTicks(normalizedInterval, min, max, startOfWeek) {
	...
	if (defined(min)) { // #1300
		if (interval >= timenuits[SECOND]) { // second
			minDate.setMilliseconds(0);
			minDate.setSeconds(interval >= timenuits[MINUTE] ? 0 :
				count * mathFloor(minDate.getSeconds() / count));
		}
	
		if (interval >= timenuits[MINUTE]) { // minute
			minDate[setMinutes](interval >= timenuits[HOUR] ? 0 :
				count * mathFloor(minDate[getMinutes]() / count));
		}
	
		if (interval >= timenuits[HOUR]) { // hour
			minDate[setHours](interval >= timenuits[DAY] ? 0 :
				count * mathFloor(minDate[getHours]() / count));
		}
	
		if (interval >= timenuits[DAY]) { // day
			minDate[setDate](interval >= timenuits[MONTH] ? 1 :
				count * mathFloor(minDate[getDate]() / count));
		}
	
		if (interval >= timenuits[MONTH]) { // month
			minDate[setMonth](interval >= timenuits[YEAR] ? 0 :
				count * mathFloor(minDate[getMonth]() / count));
			minYear = minDate[getFullYear]();
		}
	
		if (interval >= timenuits[YEAR]) { // year
			minYear -= minYear % count;
			minDate[setFullYear](minYear);
		}
	
		// week is a special case that runs outside the hierarchy
		if (interval === timenuits[WEEK]) {
		...
		while (time < max) {
			...
			if (interval === timenuits[YEAR]) {
				time = makeTime(minYear + i * count, 0);
	
			// if the interval is months, use Date.UTC to increase months
			} else if (interval === timenuits[MONTH]) {
				time = makeTime(minYear, minMonth + i * count);
	
			// if we're using global time, the interval is not fixed as it jumps
			// one hour at the DST crossover
			} else if (!useUTC && (interval === timenuits[DAY] || interval === timenuits[WEEK])) {
				time = makeTime(minYear, minMonth, minDateDate +
					i * count * (interval === timenuits[DAY] ? 1 : 7));
		...
		each(grep(tickPositions, function (time) {
			return interval <= timenuits[HOUR] && time % timenuits[DAY] === timezoneOffset;
		}), function (time) {
	...
	tooltipHeaderFormatter: function (point) {
		...
		if (isDateTime && !xDateFormat) {
			if (closestPointRange) {
				for (n in timenuits) {
					if (timenuits[n] >= closestPointRange) {
